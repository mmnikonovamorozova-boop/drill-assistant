...
def parse_passport_intellect(file_bytes, file_name):
    ...
    workload_hours = 0.0
    #  Проверка инспекции ЛНК и дефектоскопии (Синхронизация с модулем сборки КНБК)

    if any(x in full_text for x in ["трещина", "промоина", "критич. износ", "отбракован"]):
        status_lnk = "❌ Ограничение по СМК / Брак"
    elif any(x in full_text for x in ["фролов", "вахницкий", "зайцев", "михайлов", "отк", "акт"]):
        status_lnk = "✅ Годен / Контроль ЛНК пройден"
    else:
        status_lnk = "🟡 Требуется верификация ОТК"
    ...
    # Меняем верхний if на независимый, чтобы он не конфликтовал с ЛНК
    ...
    thread_patterns = {
        "4-1/2 Reg (Нижняя резьба вала шпинделя ВЗД)": (r"4-1/2\s*reg|4\s*1/2\s*reg", 2000.0, 2400.0),
        "5-1/2 FH (Верхняя резьба корпуса ВЗД)": (r"5-1/2\s*fh|5\s*1/2\s*fh", 4000.0, 4500.0),
        "NC50 / З-133 (Узел соединения КНБК)": (r"nc50|[з3][-_]?133", 2600.0, 3100.0),
        "З-117 (Резьба переводника/калибратора)": (r"[з3][-_]?117", 1800.0, 2200.0),
        "З-102 / NC38 (Малый замок переводника)": (r"[з3][-_]?102|nc38", 1200.0, 1500.0),
        "З-108 / NC40 (Муфта переводника Траектория)": (r"[з3][-_]?108|nc40", 1400.0, 1800.0),
        "З-147 (Тяжелый замок переводника)": (r"[з3][-_]?147", 3800.0, 4300.0)
    }

    # Если это ВЗД от Радиус-Сервиса и моментов в тексте нет, подстрахуем стандартными уставками корпусов
    if eq_type == "Винтовой забойный двигатель (ВЗД)" and "радиус" in vendor.lower():
        threads_matrix["Корпус ВЗД Радиус (По умолчанию)"] = {
            "min_knm": 39.2, 
            "max_knm": 44.1, 
            "label": "4000...4500 кгс·м (Стандарт завода)"
        }
    ...
    # Если текстовая марка резьбы не найдена, ищем прямую уставку в кНм
    if not threads_matrix:
        # Ищем фразу про момент, а затем забираем две пары цифр через дефис
