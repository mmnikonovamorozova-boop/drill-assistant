import json, base64, requests, numpy as np, pandas as pd, streamlit as st
...
# =========================================================================
with st.expander("🔰 Паспорт верификации СТО ИНТИ (ч.1)", expanded=False):
    st.markdown(
        "<div style='font-family: Arial, sans-serif; font-size: 13px;'>"
        "<b>1. СТО ИНТИ S.QS.7:</b> Валидация ГГИ и контроль геометрии ствола.<br>"
        "<b>2. СТО ИНТИ S.QS.8:</b> Моделирование КНБК (Minimum Curvature) и DLS.<br><br>",
        unsafe_allow_html=True
    )
    st.markdown(
        "<b>3. СТО ИНТИ S.100.3 (раздел 5.7.2):</b> Мониторинг, измерения и "
        "<b>предиктивный ИИ-анализ</b> пространственной интенсивности. Алгоритм осуществляет "
        "непрерывное вычисление геометрической невязки между строгой тригонометрической "
        "моделью API и фактическими замерами MWD телесистемы. На основе шага фильтрации "
        "система выполняет динамическую калибровку весов (K_slide и rotary_drift) "
        "для сквозного учета механического износа и радиального люфта шпинделя ВЗД "
        "при прогнозировании увода КНБК на забое."
        "</div>",
        unsafe_allow_html=True
    )
st.markdown("---")
