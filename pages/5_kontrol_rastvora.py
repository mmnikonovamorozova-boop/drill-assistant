...
# =========================================================================
# ВЕРХНЯЯ ШАПКА МОДУЛЯ: ВЕРИФИКАЦИЯ СТО ИНТИ И ПАРАМЕТРЫ КОНТРАКТА
# =========================================================================

# --- 1. КОНФИГУРАЦИЯ СТРАНИЦЫ И СТИЛИ ИНТИ ---
st.set_page_config(page_title="Контроль растворов", layout="wide")

st.title("🧪 Цифровой контроль параметров бурового раствора")
st.caption("МЕТОДИКА КОНТРОЛЯ И ОЦЕНКИ АБРАЗИВНОГО ИЗНОСА ЭЛАСТОМЕРОВ ПО РЕГЛАМЕНТАМ ПОСТАВЩИКОВ И ЗАКАЗЧИКОВ")
st.markdown("---")

# Сдержанная техническая отметка о соответствии стандартам ИНТИ с детализацией пунктов
st.markdown(
    """
    <div style='color: #4B5563; font-size: 13px; background-color: #F3F4F6; padding: 12px; border-radius: 6px; border-left: 4px solid #1E3A8A; margin-bottom: 20px; line-height: 1.5; font-family: Arial, sans-serif;'>
        ...
        <br>• <b>СТО ИНТИ S.QS.7 (п. 7.4.3 «Верификация закупаемой продукции», п. 7.5.1 «Управление производством и предоставлением услуг»)</b> в части проведения обязательного эксплуатационного аудита параметров бурового раствора, контроля содержания твердой фазы и предотвращения преждевременного износа элементов КНБК;
        <br>• <b>СТО ИНТИ S.100.3 (раздел 5.7.2 «Управление оборудованием для мониторинга и измерений», подпункт в) «Предиктивный анализ и оценка остаточного ресурса»)</b> в части математического моделирования скорости деградации нитрильных эластомеров (NBR) силовой секции ВЗД под воздействием агрессивных химических компонентов и абразивных примесей промывочной жидкости на буровой площадке.
    </div>
    """,
    ...
    with client_col2:
        # Динамический интерактивный вывод технологических лимитов
        if company_choice == "Роснефть":
            st.error(
                "📋 **Лимиты ТК «Роснефть»:**\n"
                "* Критический порог содержания песка: **> 0.5%**\n"
                "* Регламент: Обязательный замер параметров БР каждые 2 часа бурения."
            )
            max_sand_limit = 0.5
        elif company_choice == "Газпром нефть":
            st.error(
                "📋 **Лимиты ТК «Газпром нефть»:**\n"
                "* Критический порог содержания песка: **> 0.4%**\n"
                "* Регламент: Жесткий контроль ДНС и фильтрации. Ограничение расхода при росте шлама."
            )
            max_sand_limit = 0.4
            ...
            st.warning(
                "📋 **Лимиты ТК «ЛУКОЙЛ»:**\n"
                "* Критический порог содержания песка: **> 0.5%**\n"
                "* Регламент: Контроль забойной температуры и смазочной способности раствора."
            )
            max_sand_limit = 0.5
        else:
            st.info(
                "📋 **Стандартный регламент РД:**\n"
                "* Критический порог содержания песка: **> 0.5%**\n"
                "* Регламент: Контроль параметров согласно программе промывки."
            )
            max_sand_limit = 0.5
