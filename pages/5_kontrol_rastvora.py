...
# --- ИНТЕГРАЦИЯ В ШАПКУ: СЕРТИФИКАЦИЯ И ВЕРИФИКАЦИЯ СТАНДАРТОВ (БЕЗОПАСНАЯ) ---
with st.expander("🔐 Протокол соответствия ИИ-ядра стандартам ИНТИ и API (Кликните для раскрытия)"):
    
    # 1. Безопасное извлечение переменных с дефолтами для защиты от NameError
    v_temp = current_temp_est if 'current_temp_est' in locals() else 75.0
    v_sand = sand_input_val if 'sand_input_val' in locals() else 0.5
    
    # 2. Логика отображения с использованием дефолтов
    st.markdown("🔹 **Гидравлика:** API RP 13D (модель Гершеля-Балкли).")
    
    if v_temp > 70.0:
        st.info(f"⚠️ **Термический аудит (СТО ИНТИ):** Забойная температура ({v_temp}°C) > 70°C. Включен расчет теплового старения.")
    else:
        st.markdown(f"🔹 **Термический аудит (СТО ИНТИ):** Температура {v_temp}°C в норме.")
        
    if v_sand > 0.5:
        st.error(f"❌ **Абразивный аудит:** Содержание песка ({v_sand}%) превышает 0.5%!")
    else:
        st.markdown(f"🔹 **Абразивный аудит:** Песок ({v_sand}%) в пределах нормы.")
        
    st.success("🎯 **Заключение:** Алгоритмы верифицированы.")
