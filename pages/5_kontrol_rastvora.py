...
with st.expander("🔰 Паспорт верификации СТО ИНТИ", expanded=False):
    ...
    st.markdown("""
        st.markdown("""
    * **Моделирование деградации эластомеров:** Математический расчет скорости старения и термической деструкции нитрильных обкладок (NBR) статора бурового мотора производится по модифицированному уравнению Аррениуса-Крагельского с учетом накопленной циклической усталости.
    * **Контроль твердой фазы:** Строгий аудит предельного содержания абразивной фазы (песка) в промывочной жидкости для исключения гидроабразивного смыва статора.
    * **Гидродинамическое соответствие:** Валидация расчетной ЭЦП (ECD) по неньютоновской модели Гершеля-Балкли для защиты пластов от гидроразрыва (ГРП) по требованиям ТК.
    * **Синхронизация параметров:** Автоматический сквозной экспорт плотности раствора в смежные модули контроля износа опор шпинделя ВЗД.
    """)
