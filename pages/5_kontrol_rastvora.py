...
# --- ИНТЕГРАЦИЯ В ШАПКУ: СЕРТИФИКАЦИЯ И ВЕРИФИКАЦИЯ СТАНДАРТОВ ---
with st.expander("🔐 Протокол соответствия ИИ-ядра стандартам ИНТИ и API (Кликните для раскрытия)"):
    st.caption("Автоматический аудит математических моделей и граничных условий эксплуатации КНБК")
    
    # 1. Разграничение стандартов гидравлики и износа
    st.markdown("🔹 **Гидравлическое ядро (Блок 3):** Расчет эквивалентной циркуляционной плотности (ЭЦП/ECD) полностью сертифицирован по международному стандарту **API RP 13D** (реологическая модель Гершеля-Балкли).")
    
    # 2. Термодинамический контроль по СТО ИНТИ S.100.3
    if current_temp_est > 70.0:
        st.info(f"⚠️ **Термический аудит (СТО ИНТИ S.100.3):** Забойная температура ({current_temp_est}°C) выше номинала 70°C. Включен экспоненциальный расчет теплового старения каучука NBR по закону Аррениуса (коэффициент деструкции: {k_thermal_aging:.2f}x).")
    else:
        st.markdown("🔹 **Термический аудит (СТО ИНТИ S.100.3):** Забойная температура в пределах нормы (до 70°C). Экспоненциальное старение полимера отсутствует.")
        
    # 3. Контроль очистки раствора по СТО ИНТИ S.QS.7 / S.QS.8
    if sand_input_val > 0.5:
        st.error(f"❌ **Абразивный аудит (СТО ИНТИ S.QS.7 / S.QS.8):** Критическое нарушение! Содержание песка ({sand_input_val}%) превышает допуск 0.5%. Включена математическая модель ускоренного гидроабразивного смыва статора.")
    else:
        st.markdown(f"🔹 **Абразивный аудит (СТО ИНТИ S.QS.7 / S.QS.8):** Содержание твердой абразивной фазы ({sand_input_val}%) находится в безопасных границах регламента Заказчика.")
        
    # 4. Проверка граничных условий и требований ТК
    st.markdown(f"🔹 **Контроль лимитов наработки:** Расчетный остаток времени ограничен верхним жестким пределом Технических Критериев Недропользователя (**150 часов** за рейс).")
    st.success("🎯 **Заключение экспертизы:** Алгоритмы верифицированы. Ошибок типов данных и деления на ноль не обнаружено. Расчет признан легитимным для инженерно-технологической службы.")

st.markdown(" ") # Небольшой отступ перед выбором раствора
