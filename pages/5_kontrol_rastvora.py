...
# 1. Официальный верификационный баннер СТО ИНТИ
with st.container(border=True):
    header_col1, header_col2 = st.columns([1, 4])
    with header_col1:
        # Крупный визуальный маркер соответствия стандарту
        st.markdown(
            "<h1 style='text-align: center; color: #1E3A8A; margin: 0; padding: 0;'>INTI</h1>", 
            unsafe_allow_html=True
        )
    with header_col2:
        st.markdown("##### 🛡️ ЦИФРОВОЙ МОДУЛЬ ТЕХНОЛОГИЧЕСКОГО КОНТРОЛЯ БУРОВЫХ РАСТВОРОВ")
        st.caption("Верифицировано на соответствие требованиям СТО ИНТИ S.100.3-2024 «Телеметрические системы и элементы КНБК»")

st.markdown(" ")

# 2. Вынесенный из боковой панели блок недропользователя и динамических лимитов
with st.container(border=True):
    st.markdown("##### 🏢 Параметры контракта и требования Заказчика")
    client_col1, client_col2 = st.columns([2, 3])
