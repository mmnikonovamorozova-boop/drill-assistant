...
# 2. Обновленный развернутый Паспорт верификации СТО ИНТИ
with st.expander("🔰 Паспорт верификации СТО ИНТИ", expanded=False):
    st.markdown("### 🔬 Требования отраслевых стандартов СТО ИНТИ S.100.3 и S.QS.7 / S.QS.8:")
    st.markdown("* **Моделирование деградации эластомеров:** Математический расчет скорости старения и термической деструкции нитрильных обкладок (NBR) статора бурового мотора производится по модифицированному уравнению Аррениуса-Крагельского с учетом накопленной циклической усталости.")
    st.markdown("* **Контроль твердой фазы:** Строгий аудит предельного содержания абразивной фазы (песка) в промывочной жидкости для исключения гидроабразивного смыва статора.")
    st.markdown("* **Гидродинамическое соответствие:** Валидация расчетной ЭЦП (ECD) по неньютоновской модели Гершеля-Балкли для защиты пластов от гидроразрыва (ГРП) по требованиям ТК.")
    st.markdown("* **Синхронизация параметров:** Автоматический сквозной экспорт плотности раствора в смежные модули контроля износа опор шпинделя ВЗД.")
