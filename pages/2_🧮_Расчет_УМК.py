...
if vzd_key_type == "Ввести плечо вручную рулеткой":
    ...
    fact_l = st.number_input(f"2. Фактическая длина плеча ключа L (по паспорту), м:", value=base_l, step=0.01)
...
if loss_percent > 10.0:
    st.error("🚨 КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО пытаться 'дотянуть' резьбу завышением давления! Потери превышают критический лимит 10%. Остановите работы и потребуйте перестановки натяжной лебедки буровой установки под угол 90°.")
else:
    st.success("✔️ Величина погрешности находится в пределах технологического диапазона ИНТИ. Момент признан контролируемым.")
...
html_print += "<p style='font-size:15px;'><b>Фактические параметры линии:</b> Плечо УМК = " + f"{fact_l:.2f}" + " м, Толщина троса = " + f"{tros_d:.1f}" + " мм, Угол = " + f"{angle_alpha:.1f}" + "°</p>"
