...
with st.expander("🔐 Реестр легитимности и Интерактивная верификация ПО"):
    st.markdown("""
    ### 🛡️ МОДУЛЬ НЕЗАВИСИМОЙ ЭКСПРЕСС-ВЕРИФИКАЦИИ ПО
    Если у контролирующих органов возникают сомнения в точности автоматического расчета, вы можете провести независимую перепроверку математического ядра «на лету».
    """)
    
    # [Блок кода содержит вычисления с st.number_input, math.sin и метрики st.metric, 
    # а также st.success и статический markdown, детально описанные в исходном файле]
    
    st.markdown("...") # (условное отображение результатов)
    ...
    **Официальный статус ПО:**  
    ...
    st.markdown(
        "<div style='...'>ПОДТВЕРЖДЕНО • ОСМК ООО 'ТРАЕКТОРИЯ - СЕРВИС'<br>...</div>", 
