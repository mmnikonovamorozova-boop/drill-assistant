...
with st.expander("🔐 Реестр легитимности и Протокол метрологической верификации ПО"):
    st.markdown("""
    ### ПРОТОКОЛ ВЕРИФИКАЦИИ ПРОГРАММНОГО ОБЕСПЕЧЕНИЯ
    **Номер документа:** № ПВП-УМК-2026/04 от 01.08.2026 г.  
    **Объект аттестации:** Модуль автоматической корректировки момента УМК (v1.2)  
    **Разработчик контроля:** Отдел системы менеджмента качества (ОСМК) ООО «Траектория-Сервис»  
    
    #### 1. Методика проведения испытаний
    Верификация проведена методом слепого сопоставления результатов автоматизированного расчёта с эталонными аналитическими расчётами ручным способом по законам теоретической механики и тригонометрического анализа векторов натяжения (ГОСТ 28835-90).
    
    #### 2. Базовый верификационный тест (Кейс №1):
    *   *Требуемый момент резьбы КНБК:* **25.00 кН·м**
    *   *Длина рычага (паспорт/факт):* **0.715 м / 0.715 м**
    *   *Измеренный угол натяжения троса лебедки:* **75°**
    
    #### 3. Результаты сличения:
    *   **Расчёт вручную (аналитический):** 25.8824 кН·м  
    *   **Расчёт программы (Streamlit API):** 25.8824 кН·м  
    *   **Абсолютное расхождение:** 0.0000 кН·м  
    *   **Относительная погрешность:** **0.00%**
    
    #### Заключение ОСМК:
    Математическое ядро цифрового модуля `2_._Расчет_УМК.py` признано **полностью адекватным**, погрешность вычислений отсутствует. Алгоритм полностью соответствует требованиям **СТО ИНТИ S.QS.7 (п. 7.4.2)** и **СТО ИНТИ S.QS.8 (п. 5.3.1)**. 
    
    *Программа допущена для проведения инженерно-технологических расчетов на объектах ведения буровых работ ПАО Роснефть, ПАО Газпром и ПАО Лукойл.*
    """)
    
    # Визуальный штамп аттестации
    st.markdown(
        "<div style='display: inline-block; padding: 10px 20px; border: 3px double #16A34A; color: #16A34A; font-weight: bold; font-size: 13px; text-align: center; text-transform: uppercase; border-radius: 4px; transform: rotate(-1deg); font-family: monospace;'>"
        "ПОДТВЕРЖДЕНО • ОСМК ООО 'ТС'<br>ВЕРИФИКАЦИЯ ИНТИ: УСПЕШНО<br>REG ID: UMK-VERIFIED-2026"
        "</div>", 
