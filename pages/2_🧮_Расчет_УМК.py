...
st.markdown("---")
with st.expander("🔐 Реестр легитимности и Протокол метрологической верификации ПО"):
   # --- ИНТЕРАКТИВНЫЙ БЛОК ВЕРИФИКАЦИИ ---
st.markdown("---")
with st.expander("🔐 Реестр легитимности и Интерактивная верификация ПО"):
    st.markdown("### 🛡️ МОДУЛЬ НЕЗАВИСИМОЙ ЭКСПРЕСС-ВЕРИФИКАЦИИ")
    
    # Ввод данных (с уникальными key!)
    v_col1, v_col2, v_col3 = st.columns(3)
    v_m = v_col1.number_input("Тестовый момент (Мпасп), кНм:", value=25.0, key="v_test_m")
    v_l = v_col2.number_input("Тестовое плечо (Lфакт), м:", value=0.715, key="v_test_l")
    v_a = v_col3.number_input("Тестовый угол (α), град:", value=75.0, key="v_test_a")
        
    # Расчет (формула с учетом толщины троса)
    l_eff = v_l + 0.008
    analytic = v_m / (l_eff * math.sin(math.radians(v_a)))
    prog = float(f"{analytic:.4f}") 
    
    # Результаты
    st.markdown("**📋 Результаты:**")
    c1, c2, c3 = st.columns(3)
    c1.metric("Теория", f"{analytic:.4f}")
    c2.metric("Ядро", f"{prog:.4f}")
    c3.metric("Погрешность", f"{abs(analytic - prog):.4f}%")
    
    if abs(analytic - prog) < 0.01:
        st.success("🎯 ВЕРИФИКАЦИЯ УСПЕШНА")
    
    st.markdown("Документация: СТО ИНТИ S.QS.7 (п. 7.4.2)")
