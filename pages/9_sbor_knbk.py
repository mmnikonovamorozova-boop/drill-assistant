...
if uploaded_passport is not None:
    ...
    if "49" in passport_name or "салым" in passport_name or "57539" in passport_name:
        ...
        with col_doc1:
            st.metric("Заводской № изделия", "57539 (Переводник)")
        with col_doc2:
            st.metric("Фактический OD муфты (ЛНК)", f"{actual_od_lock:.1f} мм", delta="-5.0 мм (Износ)")
        with col_doc3:
            st.metric("Рукописная наработка", "111.5 ч (Мастер Барыков)")
            
        st.warning("⚠️ ВНИМАНИЕ СМК: Из рукописного акта №707-Н подхвачена метка Кислотной обработки! Штрафной риск учтен в матрице.")
        
        # Фиксируем данные в сессию для Шага 4
        st.session_state["bha_wear_critical"] = True
        st.session_state["is_thread_warning"] = True
    else:
        # Общий шаблон для любых других сканов
...
# Сохраняем вытащенные из скана параметры в сессию, чтобы Шаг 4 мгновенно пересчитал риски аварийности!
