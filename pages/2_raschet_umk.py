...
with tab_tongs:
    ...
    with col_t2:
        ...
        # === Вкладка 2: Характеристики бурильных труб (Двухмерная матрица СМК) ===
        with tab_pipe:
            ...
            # Связываем калибр КНБК из сессии (или дефолтный) с типом резьбы и габаритом
            current_caliber = st.session_state.get("bha_caliber", "Средний калибр (215.9 мм)")
            
            # Двухмерная матрица номинальных моментов ГОСТ/API [Сталь][Габарит]
            torque_matrix = {
            ...
            # ИНТЕГРАЦИОННЫЙ ШЛЮЗ: Если Адаптер КНБК выдал скорректированный ИИ-момент (износ + мороз) - берем его!
