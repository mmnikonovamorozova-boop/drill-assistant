...
with st.container(border=True):
    st.markdown(
        ...
        '🛡️ <b>Верификация:</b> Модуль соответствует <b>СТО ИНТИ S.QS.7</b> (мониторинг) и <b>СТО ИНТИ S.100.3</b> (надежность).'
        '</div>',
        unsafe_allow_html=True
    )
    
    with st.expander("🔍 Карта соответствия (Compliance Matrix)"):
        st.markdown("""
        * **СТО ИНТИ S.QS.7:** Кросс-анализ замеров ГТИ (плотность, твердая фаза) с проектными уставками.
        * **СТО ИНТИ S.100.3:** Расчет износа ВЗД от времени циркуляции и концентрации шлама.
        * **Базис:** API RP 13D / ГОСТ ISO 10414.
        """)
