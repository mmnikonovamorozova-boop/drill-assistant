...
# 1. Справочники регламентных ограничений Заказчиков (мм)
client_limits_db = {
...
# 2. Паспортная база данных заводов (предельный износ, мм)
base_vzd = {
    "Радиус-Сервис": {"43 мм": 6.0, "127 мм": 8.0, "240 мм": 10.0}, # Пример данных
    "Гидробур-Сервис": {"95 мм": 1.0, "240 мм": 1.0},
    "NOV": {"5''": 7.94, "8''": 12.70}, # Упрощено для примера
    "ВНИИБТ": {"Д-43": 1.5, "ДГР-240М": 6.0}
}
...
# Логика для американских двигателей (с конвертацией дюймов)
if selected_brand == "NOV":
    st.warning("🇺🇸 ВЗД NOV: Паспортные лимиты в метрической системе.")
    col_num, col_den = st.columns(2)
    with col_num:
        numerator = st.selectbox("Числитель:", [1, 3, 5, 7], index=3, key="b4_nov_num")
    with col_den:
        denominator = st.selectbox("Знаменатель:", [2, 4, 8, 16], index=3, key="b4_nov_den")
    mm_result = (numerator / denominator) * 25.4
    st.success(f"📐 Результат: **{mm_result:.2f} мм**")
