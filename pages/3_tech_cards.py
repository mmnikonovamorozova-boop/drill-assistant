...
if st.button("📝 Сформировать Рапорт ликвидации технологического брака"):
    lines = [
        ...
        # ... дальнейшие строки сбора данных и результатов расчетов
    ]
        "========================================",
        f"Тип инцидента: {title_text}",
        f"Нормативная база: {inti_text}",
        "----------------------------------------",
        "РЕЗУЛЬТАТЫ СВИНЧИВАНИЯ КНБК (ДАННЫЕ МОДУЛЯ 2):",
        f" Выбранная модель ключа УМК: {st.session_state.get('selected_key', 'Не выбрана')}",
        f" Выбранный тип смазки резьбы: {st.session_state.get('grease_type', 'Не указан')}",
        "----------------------------------------",
        "РЕЗУЛЬТАТЫ ПОШАГОВОЙ ПОЛЕВОЙ ВЕРИФИКАЦИИ МАРШРУТА:",
        ""
    ]

    # Добавляем данные по каждому шагу верификации маршрута из цикла
    ...
    lines.extend([
        "========================================",
        "Подписи сторон на устье скважины:",
        "",
        "Инженер по ННБ: _______________________",
        "",
        "Представитель Бурового подрядчика: _______________________",
        "",
        "Супервайзер Заказчика: _______________________"
    ])
