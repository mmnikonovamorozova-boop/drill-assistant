                ...
                if file.endswith(".png"):
                    ...
                    if not v_route:
                        ...
                        if "ясс" in card_name_lower or "jar" in file_key:
                            v_route = [
                                {"step": "Фиксация веса инструмента на крюке перед активацией ясса", "role": "Инженер ННБ"},
                                {"step": "Расчет критической силы взведения и калибровка задержки гидромеханизма", "role": "Инженер ННБ"},
                                {"step": "Контроль работы телеметрической системы MWD во время ударов", "role": "Инженер MWD"},
                                {"step": "Запись времени и величины натяжения в журнал рейса", "role": "Инженер ННБ"}
                            ]
                        else:
                            v_route = [
                                {"step": f"Проверка соответствия параметров схеме **{card_name}**", "role": "Инженер ННБ"},
                                {"step": f"Проверка датчиков наземного комплекса и каналов связи с ОЦБ", "role": "Инженер MWD"},
                                {"step": f"Фиксация результатов операции в рапорте СМО СТО ИНТИ", "role": "Супервайзер"}
                            ]
...
if route_steps and isinstance(route_steps, list):
    ...
    # Отображаем компактную интерактивную таблицу с автопереносом текста
...
if selected_client == "Без надстройки (базовый регламент)":
    ...
    if linked_requirements:
        ...
        # ФОЛБЭК: Если строгое совпадение по имени файла не сработало, берем все требования для этой операции
        ...
        if client_specific_reqs:
            ...
            st.write("ℹ️ В... В Матрице ЛНД для данного заказчика нет специфических пунктов по этой операции.")
    else:
